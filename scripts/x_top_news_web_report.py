...
STATE_PATH = REPORTS / '.x_top_seen_ids.json'
...
def infer_categories(posts):
    topic_rules = {
        'AI 產品與模型': [r'\bai\b', r'gpt', r'claude', r'gemini', r'llm', r'model', r'agent', r'openai'],
        '晶片與硬體': [r'nvidia', r'gpu', r'chip', r'cuda', r'半導體', r'晶片', r'伺服器'],
        '創業與商業策略': [r'startup', r'funding', r'vc', r'市場', r'商業', r'營收', r'growth', r'saas'],
        '政策與監管': [r'law', r'regulat', r'政策', r'監管', r'政府', r'privacy', r'copyright'],
        '安全與風險': [r'security', r'漏洞', r'攻擊', r'詐騙', r'風險', r'安全'],
        '平台與社群動態': [r'x.com', r'twitter', r'trend', r'viral', r'社群', r'貼文', r'creator'],
    }
    ...
    for p in posts:
        ...
        for topic, patterns in topic_rules.items():
            ...
            if score > score_best:
                score_best = score
                picked = topic
        buckets[picked or '其他焦點'].append(p)

    # ensure exactly 6 categories by volume, naming from discovered buckets
    ordered = sorted(buckets.items(), key=lambda kv: len(kv[1]), reverse=True)
    names = [k for k, _ in ordered]
    while len(names) < 6:
        names.append(f'焦點補位 {len(names)+1}')
        buckets[names[-1]] = []
    names = names[:6]

    # merge overflow buckets into nearest generic bucket
...
async def scrape_top_80():
    ...
    try:
        async with async_playwright() as p:
            ...
            url = f"https://x.com/search?q={quote('AI OR OpenAI OR NVIDIA OR startup OR regulation')}&src=typed_query&f=top"
...
def render_html(ts, categories, insights, blocked_reason=''):
    lines = []
    line_no = 1
    for cat, items in categories:
        lines.append(f"<section class='cat'><h2>{cat}</h2>")
        if not items:
            lines.append("<p class='empty'>本輪無新增貼文</p>")
        for p in items:
            ...
            lines.append(
                f"<div class='line' id='L{line_no}' data-line='{line_no}' data-copy='{text} | 原文：{url}'>"
                f"<a class='ln' href='#L{line_no}' title='複製此行 highlight link'>{line_no:03d}</a> "
                f"<span class='txt' title='點文字複製：該行文字+原文連結'>{text}</span> "
                f"<a class='tid' href='{url}' target='_blank' rel='noopener' title='開啟原貼文'>{tid}</a> "
    ...
    blocked_html = f"<div class='blocked'>⚠️ 抓取受阻：{blocked_reason}</div>" if blocked_reason else ''
...
def render_md(ts, categories, insights, blocked_reason=''):
    out = [
    ...
    for cat, items in categories:
        ...
        for p in items:
            out.append(
                f"{line_no:03d}. {rewrite_summary(p)} ({p['url']}) [{p['tweet_id']}]"
                f"  @ {p.get('author','')} | {p.get('time','')} | ❤{p.get('likes','0')} 🔁{p.get('reposts','0')} 💬{p.get('replies','0')}"
            )
            line_no += 1
        out.append('')
...
async def main():
    ...
    html_path = WEB_DIR / f'x_top_news_{ts_file}.html'
    md_path = REPORTS / f'x_top_news_{ts_file}.md'
    ...
    insights = [
        f"本輪 Top 分頁滾動 80 次後，共新增 {len(new_posts)} 則可追蹤貼文（去重後）。",
        '高互動貼文集中在 AI 產品發布與商業化敘事，話題具有快速擴散特性。',
        '建議優先追蹤高互動原文，於 24 小時內完成二次解讀與對外內容轉譯。',
    ]
    if blocked_reason and not new_posts:
        insights = [
            '本輪受登入/風控限制，未能完整讀取貼文內容。',
            '已仍照網站版型產出，便於後續補抓與版本追蹤。',
            '建議先恢復 X 連線狀態後重跑同流程。',
        ]

    html = render_html(ts_human, categories, insights, blocked_reason)
    md = render_md(ts_human, categories, insights, blocked_reason)
