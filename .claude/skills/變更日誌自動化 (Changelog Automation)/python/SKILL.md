---
name: 變更日誌自動化 (Changelog Automation)/python
description: "Use Case #074 Python 方案: 變更日誌自動化。使用 Python 實作 Changelog Automation 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #074: 變更日誌自動化 — Python 方案
...
```bash
mkdir -p changelog-automation && cd changelog-automation
...
根據 變更日誌自動化 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Changelog Automation"""
import anthropic
...
def run():
    print(f"=== 變更日誌自動化 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 變更日誌自動化 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/changelog-automation.log 2>&1
```
