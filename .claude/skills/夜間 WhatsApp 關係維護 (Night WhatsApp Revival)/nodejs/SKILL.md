---
name: 夜間 WhatsApp 關係維護 (Night WhatsApp Revival)/nodejs
description: "Use Case #147 Node.js 方案: 夜間 WhatsApp 關係維護。使用 Node.js 實作 Night WhatsApp Revival 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(npx *), Bash(node *), Bash(mkdir *), Bash(touch *)
---

# Use Case #147: 夜間 WhatsApp 關係維護 — Node.js 方案
...
建立 `index.js`，實作 夜間 WhatsApp 關係維護 的核心邏輯。
...
```bash
# 加入 crontab（每日執行）
# 0 9 * * * cd ~/night-whatsapp-revival && node index.js >> output.log 2>&1
```
