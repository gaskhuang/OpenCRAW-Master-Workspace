---
name: 自我修復家庭伺服器 (Self-Healing Home Server)/python
description: "Use Case #070 Python 方案: 自我修復家庭伺服器。使用 Python 實作 Self-Healing Home Server 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #070: 自我修復家庭伺服器 — Python 方案
...
```bash
mkdir -p self-healing-home-server && cd self-healing-home-server
...
根據 自我修復家庭伺服器 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Self-Healing Home Server"""
import anthropic
...
```python
"""Main orchestrator for Self-Healing Home Server"""
...
def run():
    print(f"=== 自我修復家庭伺服器 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 自我修復家庭伺服器 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/self-healing-home-server.log 2>&1
```
