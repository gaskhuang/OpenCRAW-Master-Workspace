---
name: X 互動管理助手 (X Engagement Manager)/python
description: "Use Case #009 Python 方案: X 互動管理助手。使用 Python 實作 X Engagement Manager 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #009: X 互動管理助手 — Python 方案
...
```bash
mkdir -p x-engagement-manager && cd x-engagement-manager
...
根據 X 互動管理助手 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for X Engagement Manager"""
import anthropic
...
```python
"""Main orchestrator for X Engagement Manager"""
...
def run():
    print(f"=== X 互動管理助手 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 X 互動管理助手 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/x-engagement-manager.log 2>&1
```
