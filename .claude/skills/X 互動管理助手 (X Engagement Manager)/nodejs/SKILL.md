---
name: X 互動管理助手 (X Engagement Manager)/nodejs
description: "Use Case #009 Node.js 方案: X 互動管理助手。使用 Node.js 實作 X Engagement Manager 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(node *), Bash(npx *), Bash(mkdir *)
---

# Use Case #009: X 互動管理助手 — Node.js 方案
...
安裝:
```bash
mkdir x-engagement-manager && cd x-engagement-manager
...
export async function collectData() {
  // TODO: Implement data collection for X Engagement Manager
...
async function run() {
  logger.info('=== X 互動管理助手 ===');
  validate();
  const data = await collectData();
  if (data) {
    const result = await analyzeWithAI(data);
    await sendTelegram(`📊 X 互動管理助手 報告\n\n${result}`);
    logger.info('✅ Done!');
  } else {
...
pm2 start src/index.js --name x-engagement-manager  # 持久化
```
