---
name: 訂閱費用審計 (Subscription Cost Audit)/python
description: "Use Case #105 Python 方案: 訂閱費用審計。使用 Python 實作 Subscription Cost Audit 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #105: 訂閱費用審計 — Python 方案
...
```bash
mkdir -p subscription-cost-audit && cd subscription-cost-audit
...
根據 訂閱費用審計 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Subscription Cost Audit"""
import anthropic
...
```python
"""Main orchestrator for Subscription Cost Audit"""
...
def run():
    print(f"=== 訂閱費用審計 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 訂閱費用審計 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/subscription-cost-audit.log 2>&1
```
