---
name: 競品定價即時追蹤 (Competitor Pricing Tracker)/python
description: "Use Case #056 Python 方案: 競品定價即時追蹤。使用 Python 實作 Competitor Pricing Tracker 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #056: 競品定價即時追蹤 — Python 方案
...
```bash
mkdir -p competitor-pricing-tracker && cd competitor-pricing-tracker
...
根據 競品定價即時追蹤 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Competitor Pricing Tracker"""
import anthropic
...
```python
"""Main orchestrator for Competitor Pricing Tracker"""
...
def run():
    print(f"=== 競品定價即時追蹤 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 競品定價即時追蹤 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/competitor-pricing-tracker.log 2>&1
```
