---
name: 市場調研與 MVP 工廠 (Market Research & MVP Factory)/python
description: "Use Case #095 Python 方案: 市場調研與 MVP 工廠。使用 Python 實作 Market Research & MVP Factory 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #095: 市場調研與 MVP 工廠 — Python 方案
...
```bash
mkdir -p market-research-&-mvp-factory && cd market-research-&-mvp-factory
...
根據 市場調研與 MVP 工廠 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Market Research & MVP Factory"""
import anthropic
...
```python
"""Main orchestrator for Market Research & MVP Factory"""
...
def run():
    print(f"=== 市場調研與 MVP 工廠 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 市場調研與 MVP 工廠 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/market-research-&-mvp-factory.log 2>&1
```
