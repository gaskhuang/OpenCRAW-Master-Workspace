---
name: 多代理內容工廠 (Multi-Agent Content Factory)/python
description: "Use Case #012 Python 方案: 多代理內容工廠。使用 Python 實作 Multi-Agent Content Factory 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #012: 多代理內容工廠 — Python 方案
...
```bash
mkdir -p multi-agent-content-factory && cd multi-agent-content-factory
...
根據 多代理內容工廠 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Multi-Agent Content Factory"""
import anthropic
...
```python
"""Main orchestrator for Multi-Agent Content Factory"""
...
def run():
    print(f"=== 多代理內容工廠 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 多代理內容工廠 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/multi-agent-content-factory.log 2>&1
```
