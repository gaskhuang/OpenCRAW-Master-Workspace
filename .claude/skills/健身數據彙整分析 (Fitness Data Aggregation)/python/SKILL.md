---
name: 健身數據彙整分析 (Fitness Data Aggregation)/python
description: "Use Case #112 Python 方案: 健身數據彙整分析。使用 Python 實作 Fitness Data Aggregation 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #112: 健身數據彙整分析 — Python 方案
...
```bash
mkdir -p fitness-data-aggregation && cd fitness-data-aggregation
...
根據 健身數據彙整分析 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Fitness Data Aggregation"""
import anthropic
...
```python
"""Main orchestrator for Fitness Data Aggregation"""
...
def run():
    print(f"=== 健身數據彙整分析 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 健身數據彙整分析 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/fitness-data-aggregation.log 2>&1
```
