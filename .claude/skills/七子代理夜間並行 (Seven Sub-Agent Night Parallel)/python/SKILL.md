---
name: 七子代理夜間並行 (Seven Sub-Agent Night Parallel)/python
description: "Use Case #145 Python 方案: 七子代理夜間並行。使用 Python 實作 Seven Sub-Agent Night Parallel 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #145: 七子代理夜間並行 — Python 方案
...
def run_seven_sub_agent_night_parallel():
    """執行 七子代理夜間並行 主流程"""
...
if __name__ == "__main__":
    result = run_seven_sub_agent_night_parallel()
    print(result)
```
...
```bash
# 加入 crontab（每日執行）
# 0 9 * * * cd ~/seven-sub-agent-night-parallel && python3 main.py >> output.log 2>&1
```
