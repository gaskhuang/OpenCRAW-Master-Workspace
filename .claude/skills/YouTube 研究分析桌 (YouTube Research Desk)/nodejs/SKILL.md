---
name: YouTube 研究分析桌 (YouTube Research Desk)/nodejs
description: "Use Case #098 Node.js 方案: YouTube 研究分析桌。使用 Node.js 實作 YouTube Research Desk 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(node *), Bash(npx *), Bash(mkdir *)
---

# Use Case #098: YouTube 研究分析桌 — Node.js 方案
...
安裝:
```bash
mkdir youtube-research-desk && cd youtube-research-desk
...
export async function collectData() {
  // TODO: Implement data collection for YouTube Research Desk
...
async function run() {
  logger.info('=== YouTube 研究分析桌 ===');
  validate();
  const data = await collectData();
  if (data) {
    const result = await analyzeWithAI(data);
    await sendTelegram(`📊 YouTube 研究分析桌 報告\n\n${result}`);
    logger.info('✅ Done!');
  } else {
...
pm2 start src/index.js --name youtube-research-desk  # 持久化
```
