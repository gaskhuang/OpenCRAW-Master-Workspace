---
name: YouTube 研究分析桌 (YouTube Research Desk)/python
description: "Use Case #098 Python 方案: YouTube 研究分析桌。使用 Python 實作 YouTube Research Desk 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #098: YouTube 研究分析桌 — Python 方案
...
```bash
mkdir -p youtube-research-desk && cd youtube-research-desk
...
根據 YouTube 研究分析桌 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for YouTube Research Desk"""
import anthropic
...
```python
"""Main orchestrator for YouTube Research Desk"""
...
def run():
    print(f"=== YouTube 研究分析桌 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 YouTube 研究分析桌 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/youtube-research-desk.log 2>&1
```
