---
name: X 帳號質性分析 (X Account Qualitative Analysis)/python
description: "Use Case #003 Python 方案: X 帳號質性分析。使用 Python 實作 X Account Qualitative Analysis 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #003: X 帳號質性分析 — Python 方案
...
```bash
mkdir -p x-account-qualitative-analysis && cd x-account-qualitative-analysis
...
根據 X 帳號質性分析 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for X Account Qualitative Analysis"""
import anthropic
...
```python
"""Main orchestrator for X Account Qualitative Analysis"""
...
def run():
    print(f"=== X 帳號質性分析 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 X 帳號質性分析 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/x-account-qualitative-analysis.log 2>&1
```
