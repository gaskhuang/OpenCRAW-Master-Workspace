---
name: 潛在客戶資料豐富 (Lead Enrichment)/python
description: "Use Case #060 Python 方案: 潛在客戶資料豐富。使用 Python 實作 Lead Enrichment 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #060: 潛在客戶資料豐富 — Python 方案
...
```bash
mkdir -p lead-enrichment && cd lead-enrichment
...
根據 潛在客戶資料豐富 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Lead Enrichment"""
import anthropic
...
def run():
    print(f"=== 潛在客戶資料豐富 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 潛在客戶資料豐富 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/lead-enrichment.log 2>&1
```
