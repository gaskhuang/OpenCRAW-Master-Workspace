---
name: 學術研究助手 (Academic Research Assistant)/python
description: "Use Case #018 Python 方案: 學術研究助手。使用 Python 實作 Academic Research Assistant 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #018: 學術研究助手 — Python 方案
...
```bash
mkdir -p academic-research-assistant && cd academic-research-assistant
...
根據 學術研究助手 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Academic Research Assistant"""
import anthropic
...
```python
"""Main orchestrator for Academic Research Assistant"""
...
def run():
    print(f"=== 學術研究助手 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 學術研究助手 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/academic-research-assistant.log 2>&1
```
