---
name: 個人 CLI 工具包 (Personal CLI Toolkit)/nodejs
description: "Use Case #143 Node.js 方案: 個人 CLI 工具包。使用 Node.js 實作 Personal CLI Toolkit 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(npx *), Bash(node *), Bash(mkdir *), Bash(touch *)
---

# Use Case #143: 個人 CLI 工具包 — Node.js 方案
...
建立 `index.js`，實作 個人 CLI 工具包 的核心邏輯。
...
```bash
# 加入 crontab（每日執行）
# 0 9 * * * cd ~/personal-cli-toolkit && node index.js >> output.log 2>&1
```
