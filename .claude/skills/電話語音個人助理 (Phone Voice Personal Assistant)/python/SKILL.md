---
name: 電話語音個人助理 (Phone Voice Personal Assistant)/python
description: "Use Case #031 Python 方案: 電話語音個人助理。使用 Python 實作 Phone Voice Personal Assistant 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #031: 電話語音個人助理 — Python 方案
...
```
phone-voice-personal-assistant/
├── .env                    # 環境變數
...
```bash
mkdir -p phone-voice-personal-assistant && cd phone-voice-personal-assistant
...
根據 電話語音個人助理 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Phone Voice Personal Assistant"""
import anthropic
...
```python
"""Main orchestrator for Phone Voice Personal Assistant"""
...
def run():
    print(f"=== 電話語音個人助理 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 電話語音個人助理 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/phone-voice-personal-assistant.log 2>&1
```
