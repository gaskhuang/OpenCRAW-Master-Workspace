---
name: 電話語音個人助理 (Phone Voice Personal Assistant)/nodejs
description: "Use Case #031 Node.js 方案: 電話語音個人助理。使用 Node.js 實作 Phone Voice Personal Assistant 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(node *), Bash(npx *), Bash(mkdir *)
---

# Use Case #031: 電話語音個人助理 — Node.js 方案
...
安裝:
```bash
mkdir phone-voice-personal-assistant && cd phone-voice-personal-assistant
...
export async function collectData() {
  // TODO: Implement data collection for Phone Voice Personal Assistant
...
async function run() {
  logger.info('=== 電話語音個人助理 ===');
  validate();
  const data = await collectData();
  if (data) {
    const result = await analyzeWithAI(data);
    await sendTelegram(`📊 電話語音個人助理 報告\n\n${result}`);
    logger.info('✅ Done!');
  } else {
...
pm2 start src/index.js --name phone-voice-personal-assistant  # 持久化
```
