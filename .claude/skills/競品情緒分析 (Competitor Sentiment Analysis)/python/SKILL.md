---
name: 競品情緒分析 (Competitor Sentiment Analysis)/python
description: "Use Case #057 Python 方案: 競品情緒分析。使用 Python 實作 Competitor Sentiment Analysis 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #057: 競品情緒分析 — Python 方案
...
```bash
mkdir -p competitor-sentiment-analysis && cd competitor-sentiment-analysis
...
根據 競品情緒分析 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Competitor Sentiment Analysis"""
import anthropic
...
```python
"""Main orchestrator for Competitor Sentiment Analysis"""
...
def run():
    print(f"=== 競品情緒分析 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 競品情緒分析 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/competitor-sentiment-analysis.log 2>&1
```
