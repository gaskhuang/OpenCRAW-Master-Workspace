---
name: Polymarket 自動交易 (Polymarket Auto Trading)/python
description: "Use Case #102 Python 方案: Polymarket 自動交易。使用 Python 實作 Polymarket Auto Trading 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #102: Polymarket 自動交易 — Python 方案
...
Use sub-agents to analyze multiple market
...
```bash
mkdir -p polymarket-auto-trading && cd polymarket-auto-trading
...
根據 Polymarket 自動交易 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Polymarket Auto Trading"""
import anthropic
...
```python
"""Main orchestrator for Polymarket Auto Trading"""
...
def run():
    print(f"=== Polymarket 自動交易 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 Polymarket 自動交易 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/polymarket-auto-trading.log 2>&1
```
