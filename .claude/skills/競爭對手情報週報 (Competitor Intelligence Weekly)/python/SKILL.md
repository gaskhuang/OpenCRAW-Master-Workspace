---
name: 競爭對手情報週報 (Competitor Intelligence Weekly)/python
description: "Use Case #055 Python 方案: 競爭對手情報週報。使用 Python 實作 Competitor Intelligence Weekly 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #055: 競爭對手情報週報 — Python 方案
...
```bash
mkdir -p competitor-intelligence-weekly && cd competitor-intelligence-weekly
...
根據 競爭對手情報週報 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Competitor Intelligence Weekly"""
import anthropic
...
```python
"""Main orchestrator for Competitor Intelligence Weekly"""
...
def run():
    print(f"=== 競爭對手情報週報 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 競爭對手情報週報 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/competitor-intelligence-weekly.log 2>&1
```
