---
name: Prompt 回歸監控 (Prompt Regression Watch)/python
description: "Use Case #126 Python 方案: Prompt 回歸監控。使用 Python 實作 Prompt Regression Watch 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #126: Prompt 回歸監控 — Python 方案
...
建立 `main.py`，實作 Prompt 回歸監控 的核心邏輯。
...
def run_prompt_regression_watch():
    """執行 Prompt 回歸監控 主流程"""
...
if __name__ == "__main__":
    result = run_prompt_regression_watch()
    print(result)
```
...
```bash
# 加入 crontab（每日執行）
# 0 9 * * * cd ~/prompt-regression-watch && python3 main.py >> output.log 2>&1
```
