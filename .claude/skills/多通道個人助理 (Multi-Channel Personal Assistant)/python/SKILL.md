---
name: 多通道個人助理 (Multi-Channel Personal Assistant)/python
description: "Use Case #035 Python 方案: 多通道個人助理。使用 Python 實作 Multi-Channel Personal Assistant 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #035: 多通道個人助理 — Python 方案
...
```bash
mkdir -p multi-channel-personal-assistant && cd multi-channel-personal-assistant
...
根據 多通道個人助理 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Multi-Channel Personal Assistant"""
import anthropic
...
```python
"""Main orchestrator for Multi-Channel Personal Assistant"""
...
def run():
    print(f"=== 多通道個人助理 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 多通道個人助理 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/multi-channel-personal-assistant.log 2>&1
```
