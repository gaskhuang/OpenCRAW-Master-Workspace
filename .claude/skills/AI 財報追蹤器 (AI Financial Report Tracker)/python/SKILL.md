---
name: AI 財報追蹤器 (AI Financial Report Tracker)/python
description: "Use Case #093 Python 方案: AI 財報追蹤器。使用 Python 實作 AI Financial Report Tracker 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #093: AI 財報追蹤器 — Python 方案
...
```bash
mkdir -p ai-financial-report-tracker && cd ai-financial-report-tracker
...
根據 AI 財報追蹤器 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for AI Financial Report Tracker"""
import anthropic
...
```python
"""Main orchestrator for AI Financial Report Tracker"""
...
def run():
    print(f"=== AI 財報追蹤器 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 AI 財報追蹤器 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/ai-financial-report-tracker.log 2>&1
```
