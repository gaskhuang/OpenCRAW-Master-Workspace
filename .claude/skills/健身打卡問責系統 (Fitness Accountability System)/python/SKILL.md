---
name: 健身打卡問責系統 (Fitness Accountability System)/python
description: "Use Case #110 Python 方案: 健身打卡問責系統。使用 Python 實作 Fitness Accountability System 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(pip *), Bash(python *), Bash(mkdir *), Bash(touch *)
---

# Use Case #110: 健身打卡問責系統 — Python 方案
...
```bash
mkdir -p fitness-accountability-system && cd fitness-accountability-system
...
根據 健身打卡問責系統 的需求，實作資料收集與處理邏輯。

```python
"""Core business logic for Fitness Accountability System"""
import anthropic
...
```python
"""Main orchestrator for Fitness Accountability System"""
...
def run():
    print(f"=== 健身打卡問責系統 - {datetime.now():%Y-%m-%d %H:%M} ===")
    Config.validate()
    data = collect_data()
    if data:
        result = analyze_with_ai(data)
        send_telegram(f"📊 健身打卡問責系統 報告\n\n{result}")
        print("✅ Done!")
    else:
...
# 每天執行
crontab -e
0 9 * * * cd /path/to/project && /path/to/venv/bin/python main.py >> /tmp/fitness-accountability-system.log 2>&1
```
