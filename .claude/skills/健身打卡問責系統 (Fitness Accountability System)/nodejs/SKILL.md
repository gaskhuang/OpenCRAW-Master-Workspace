---
name: 健身打卡問責系統 (Fitness Accountability System)/nodejs
description: "Use Case #110 Node.js 方案: 健身打卡問責系統。使用 Node.js 實作 Fitness Accountability System 自動化系統。"
allowed-tools: Read, Grep, Glob, Bash(npm *), Bash(node *), Bash(npx *), Bash(mkdir *)
---

# Use Case #110: 健身打卡問責系統 — Node.js 方案
...
安裝:
```bash
mkdir fitness-accountability-system && cd fitness-accountability-system
...
export async function collectData() {
  // TODO: Implement data collection for Fitness Accountability System
...
async function run() {
  logger.info('=== 健身打卡問責系統 ===');
  validate();
  const data = await collectData();
  if (data) {
    const result = await analyzeWithAI(data);
    await sendTelegram(`📊 健身打卡問責系統 報告\n\n${result}`);
    logger.info('✅ Done!');
  } else {
...
pm2 start src/index.js --name fitness-accountability-system  # 持久化
```
